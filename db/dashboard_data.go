...
func GetDashboardData(db *pgx.Conn, input User) ([]DashboardData, error) {
	...
	SELECT "user_resource".resource_name,"user_resource".amount,"user_resource".factory_level,"factory".production_per_second,"user_resource".time_until_upgrade_complete
	...
	for rows.Next() {
		...
		if err := rows.Scan(&data.ResourceName, &data.Amount, &data.FactoryLevel, &data.ProductionRate, &data.TimeUntilUpgradeComplete); err != nil {
			return nil, err
		}
